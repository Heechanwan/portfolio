...
function createSkyEnvironment() {
    ...
    skyMat.colorNode = mix(
        color(0x0d3b4f),  // deep teal sky
        color(0x4fd0c4),  // aqua horizon glow
        pow(max(float(0).sub(skyUV.y), float(0)), float(0.8))
    ).add(
        mix(color(0x000000), color(0xbdf2e6), max(skyUV.y, float(0)).mul(float(0.3)))
    );
    ...
    sunMat.colorNode = color(0xbdf2ee).mul(float(2.0));
...
const grassColors = ['#2fa89a', '#268f83', '#3cbcae', '#1f7b70', '#4fcabb', '#289a8c', '#33a596', '#3bb2a2'];
const rockColors = ['#8a9aa0', '#77878e', '#9dabb0', '#6d7c82', '#aebcc0', '#5f6d72', '#a3b1b5', '#748387'];
const trunkColors = ['#7a5a3c', '#68482e', '#8a6a48', '#5a3f26', '#9a7854'];
const leafColors = ['#ff7a45', '#ff8f52', '#ffab5e', '#f0602f', '#ff9d6b', '#ff6e38', '#e85a2c', '#ffb870', '#ff8358', '#f27540'];
const flowerColors = ['#ff5c8a', '#ff81a6', '#ffa0bd', '#ff3f78', '#ff9cc4', '#ff6f9e'];
...
const VARIATION_NAMES = ['Sunset Reef', 'Frost Atoll', 'Lagoon Bloom'];
...
const dirtColors = ['#3f6b6a', '#365d5c', '#2e5150', '#476f6b', '#294544', '#4f7876', '#376260'];
const stoneUndersideColors = ['#4a5a5c', '#3e4c4e', '#576a6c', '#354143', '#495a5b', '#5c7072'];
...
const canopyRadiusH = 7.0;
const canopyRadiusV = 4.0;
...
const grassTuftColors = ['#22857a', '#2a9688', '#1c6f64', '#39ab9c', '#268a7e'];
...
// === MUSHROOMS — batched ===
const mushroomColors = ['#f5f0e0', '#e8e2cc', '#d8dcc0', '#eae6d0'];
Object.entries(grassTopMap).forEach(([key, topY]) => {
    const [gx, gz] = key.split(',').map(Number);
...
function generateSnowyPineOffsets() {
    ...
    // Frost palette
    const snowGrass = ['#e6f4f5', '#d2eaed', '#c6e2e5', '#eef8f8', '#daeeef'];
    const snowRock = ['#cfe0e2', '#bcd4d6', '#dee9ea', '#aecacc', '#c6d9da'];
    const pineColors = ['#1c5a66', '#20606e', '#184e5a', '#256a78', '#1a5460', '#154650'];
    const pineTrunk = ['#3a4a4c', '#2e3c3e', '#4a5c5e', '#343f40'];
    const snowWhite = ['#f0fbff', '#e8f6ff', '#ffffff', '#f5fcff', '#eafaff'];
    const iceBlue = ['#a8d8f0', '#8fc6e8', '#7fbadf'];
...
function generateCherryBlossomOffsets() {
    ...
    // Lagoon bloom palette
    const sakuraGrass = ['#2fa89a', '#268f83', '#3cbcae', '#1f7b70', '#33a596'];
    const sakuraRock = ['#8aa8a4', '#7a9490', '#9db8b4', '#84a09c', '#5c7472'];
    const blossomPink = ['#7ee0ff', '#5ecfff', '#9ce8ff', '#4bc4f5', '#b0eeff', '#6ed8ff', '#57ccf7', '#c4f2ff'];
    const blossomWhite = ['#eafdff', '#dcf8ff', '#f2feff', '#e0faff'];
    const sakuraTrunk = ['#4a6858', '#3a5446', '#5a7868', '#2e4438', '#688a78'];
    const mossGreen = ['#38a08c', '#2c8873', '#45b09a'];
...
dustMat.colorNode = color(0xbdf2ee);
...
leafQuadMat.colorNode = color(0xff7a45);
...
const leafParticleColors = [
    ['#ff7a45', '#ff8f52', '#ffab5e', '#ff9d6b', '#f5a623', '#ff8c42'], // sunset reef
    ['#1c5a66', '#20606e', '#256a78', '#1a5460', '#154650', '#2e7688'],  // frost atoll
    ['#7ee0ff', '#5ecfff', '#9ce8ff', '#eafdff', '#b0eeff'],            // lagoon bloom
];
...
// Lighting
const ambientLight = new THREE.AmbientLight(0xdcf5f0, 0.5);
ambientLight.name = 'ambientLight';
scene.add(ambientLight);

const mainLight = new THREE.DirectionalLight(0xeafff8, 2.5);
...
const softShadowLight = new THREE.DirectionalLight(0xcdeee8, 0.6);
...
const fillLight = new THREE.DirectionalLight(0x4aa8ff, 1.0);
fillLight.name = 'fillLight';
fillLight.position.set(-5, 8, -3);
scene.add(fillLight);

const rimLight = new THREE.PointLight(0x4fe0c8, 1.5, 30);
rimLight.name = 'rimLight';
rimLight.position.set(-4, 12, -5);
scene.add(rimLight);

const accentLight = new THREE.PointLight(0xff7a55, 1.2, 25);
...
const defaultSwatch = document.querySelector('.color-swatch[data-color="#0d3b4f"]');
